...
*/
	...
	public JdbcBatchItemWriter<Pensioner> jdbcBatchItemWriter(DataSource dataSource,
		...
		jdbcBatchItemWriter.setSql("insert into pensioners(aadhaar_number,name,dob,salary_earned,allowance,type_selforfamilypension,basic_salary,pensioner_account_number,pancard,user_name,password) "
				+"values (:aadhaarNumber, :name, :dob, :salaryEarned, :allowance, :typeSelfORFamilypension, :basicSalary, :pensionerAccountNumber, :pancard, :userName, :password)");
	...
	public Step step1(StepBuilderFactory stepBuilderFactory, ResourcelessTransactionManager transactionManager,
		...
		return stepBuilderFactory.get("step1").transactionManager(transactionManager).<Pensioner,Pensioner>chunk(10)
